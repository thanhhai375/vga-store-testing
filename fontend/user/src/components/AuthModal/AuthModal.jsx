...
const AuthModal = ({ isOpen, onClose }) => {
  ...
  const handleOverlayClick = () => {
    if (hasDirtyForm()) {
      toastInfo('Bạn đang nhập dữ liệu, vui lòng hoàn tất hoặc bấm nút đóng.');
      return;
    }
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        ...
        const userObj = payload.user || userInfo;
        const finalPayload = {
          ...payload,
          user: userObj
        };

        saveAuthStorage(finalPayload);
        ...
        toastError(
          error.response?.data?.message ||
            'Hệ thống bảo mật từ chối đăng nhập Google tạm thời!'
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let errorMsg =
        ...
        err.message ||
        (isLogin ? 'Tài khoản hoặc mật khẩu không đúng!' : 'Đăng ký thất bại!');
        ...
        <div className="auth-body">
          <form className="auth-form" onSubmit={handleSubmit}>
            ...
            <div className="auth-input-group">
              ...
              <input
                ...
                placeholder={isLogin ? 'Nhập tài khoản hoặc email' : 'Nhập email của bạn'}
                value={email}
          ...
          <div className="auth-social">
            ...
            <button
              ...
              disabled
              title="Facebook login chưa hỗ trợ"
              aria-disabled="true"
            >
              <svg
                viewBox="0 0 24 24"
