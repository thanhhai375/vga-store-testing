...
// ============================================================
// NHOM G - Luong E2E co thay doi du lieu (chi chay khi duoc cho phep)
// ============================================================

Scenario(
  'ORD_UI_25 - Optional: Xac nhan thanh toan va chuyen sang Hoan thanh',
  async ({ I }) => {
    if (!ALLOW_ORDER_MUTATION) {
      I.say(
        'Bo qua cap nhat that. Dat ORDER_E2E_ALLOW_UPDATE=true neu duoc phep thay doi du lieu.'
      );
      return;
    }

    // Buoc 1: Tim mot don PENDING + UNPAID.
    await selectStatusFilter(I, 'PENDING');

    const paymentButtonCount = await I.grabNumberOfVisibleElements(
      SELECTORS.table.confirmPaymentButtons
    );

    if (paymentButtonCount === 0) {
      I.say('Khong co don PENDING + UNPAID cho luong cap nhat that.');
      return;
    }

    const pendingRow = locate(SELECTORS.table.pendingPaymentRows).first();
    const orderCode = normalizeText(
      await I.grabTextFrom(pendingRow.find('td:nth-child(1) strong'))
    );

    // Buoc 2: Xac nhan da nhan tien.
    I.click(locate(SELECTORS.table.confirmPaymentButtons).first());
    I.waitForElement(SELECTORS.sweetAlert.popup, 5);
    I.click(SELECTORS.sweetAlert.confirm);
    I.waitForInvisible(SELECTORS.sweetAlert.popup, 10);
    await waitForOrdersTable(I);

    // Buoc 3: Kiem tra don da chuyen sang CONFIRMED.
    await selectStatusFilter(I, 'CONFIRMED');
    I.see(orderCode, SELECTORS.table.root);

    const targetRow = locate(SELECTORS.table.rows).withText(orderCode);
    const targetSelect = targetRow.find('select.status-select');

    I.seeElement(targetSelect);

    // Buoc 4: Chuyen don sang Hoan thanh.
    I.selectOption(targetSelect, 'DELIVERED');
    await waitForOrdersTable(I);

    // Buoc 5: Loc Hoan thanh va kiem tra lai ma don.
    await selectStatusFilter(I, 'DELIVERED');
    I.see(orderCode, SELECTORS.table.root);

    const deliveredRow = locate(SELECTORS.table.rows).withText(orderCode);
    I.seeElement(deliveredRow.find('select.status-select:disabled'));
  }
);
