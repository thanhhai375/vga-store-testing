...
Scenario('TC_PROD_025: Admin thêm mới sản phẩm VGA thành công', async ({ I }) => {
  I.amOnPage('http://localhost:5174/products/new');
  I.waitForElement('.form-group', 10);

  // Dùng timestamp để tên luôn unique, tránh trùng
...
Scenario('TC_PROD_027: Admin cập nhật thông tin sản phẩm thành công', async ({ I }) => {
...
Scenario('TC_PROD_029: Admin xóa sản phẩm thành công', async ({ I }) => {
  // Tạo sản phẩm mới để xóa — tránh xóa sản phẩm dùng cho test khác
  I.amOnPage('http://localhost:5174/products/new');
  I.waitForElement('.form-group', 10);
  ...
  // Quay về list — không search, xóa sản phẩm đầu tiên (vừa tạo sẽ ở đầu danh sách mới nhất)
  I.amOnPage('http://localhost:5174/products');
  ...
  // Kỳ vọng: SweetAlert toast thành công hoặc sản phẩm không còn ở đầu danh sách
