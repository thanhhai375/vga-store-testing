...
// Admin URL: ' + (process.env.ADMIN_FE_URL || 'http://localhost:5174') + '
// Coverage:
...
Before(({ I }) => {
  I.loginAsAdmin();
  I.amOnPage((process.env.ADMIN_FE_URL || 'http://localhost:5174') + '/products');
...
Scenario('TC_PROD_025: Admin thêm mới sản phẩm VGA thành công', ({ I }) => {
  I.amOnPage((process.env.ADMIN_FE_URL || 'http://localhost:5174') + '/products/new');
...
Scenario('TC_PROD_028: Admin truy cập trang sửa sản phẩm không tồn tại → lỗi', async ({ I }) => {
  ...
  I.amOnPage((process.env.ADMIN_FE_URL || 'http://localhost:5174') + '/products/99999/edit');
  I.wait(3);
...
Scenario('TC_PROD_031: BVA – Lọc giá hợp lệ minPrice=0, maxPrice=20tr → có kết quả', async ({ I }) => {
  ...
  I.amOnPage((process.env.USER_FE_URL || 'http://localhost:5173') + '/products?minPrice=0&maxPrice=20000000');
  I.wait(2);
...
Scenario('TC_PROD_032: BVA – minPrice âm → báo lỗi hoặc không có kết quả', ({ I }) => {
  I.amOnPage((process.env.USER_FE_URL || 'http://localhost:5173') + '/products?minPrice=-1500000');
  I.wait(3);
...
Scenario('TC_PROD_033: BVA – maxPrice âm → báo lỗi hoặc không có kết quả', ({ I }) => {
  I.amOnPage((process.env.USER_FE_URL || 'http://localhost:5173') + '/products?maxPrice=-5000000');
  I.wait(3);
  I.seeElement('.shop-product-grid');
});

Scenario('TC_PROD_034: BVA – minPrice > maxPrice → kết quả rỗng hoặc báo lỗi', ({ I }) => {
  I.amOnPage((process.env.USER_FE_URL || 'http://localhost:5173') + '/products?minPrice=20000000&maxPrice=5000000');
  I.wait(3);
  I.seeElement('.shop-product-grid');
});

Scenario('TC_PROD_035: BVA – maxPrice=0 → danh sách rỗng hoặc sản phẩm giá 0đ', ({ I }) => {
  I.amOnPage((process.env.USER_FE_URL || 'http://localhost:5173') + '/products?minPrice=0&maxPrice=0');
  I.wait(3);
...
Scenario('TC_PROD_036: BVA – minPrice Overflow → không crash hệ thống', ({ I }) => {
  I.amOnPage((process.env.USER_FE_URL || 'http://localhost:5173') + '/products?minPrice=9999999999999999');
  I.wait(3);
  I.dontSee('Lỗi hệ thống');
  I.seeElement('.shop-product-grid');
});
...
Scenario('TC_PROD_037: BVA – maxPrice Overflow → không crash hệ thống', ({ I }) => {
  I.amOnPage((process.env.USER_FE_URL || 'http://localhost:5173') + '/products?maxPrice=9999999999999999');
  I.wait(3);
  I.dontSee('Lỗi hệ thống');
  I.seeElement('.shop-product-grid');
});
