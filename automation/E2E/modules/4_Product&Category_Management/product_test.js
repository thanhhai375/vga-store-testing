...
// Base URL: ' + (process.env.USER_FE_URL || 'http://localhost:5173') + '
...
async function getFirstProductId(I) {
  ...
  const id = await I.executeScript(async (apiBaseUrl) => {
    try {
      const res = await fetch(`${apiBaseUrl}/api/products?size=1&page=0`);
...
Scenario('TC_PROD_021: Thông số kỹ thuật hiển thị trong trang chi tiết', async ({ I }) => {
  ...
  I.amOnPage((process.env.USER_FE_URL || 'http://localhost:5173') + `/product/${id}`);
  I.waitForElement('.product-detail-page', 10);
...
Scenario('TC_PROD_024: Sản phẩm không tồn tại hiển thị lỗi', ({ I }) => {
  I.amOnPage((process.env.USER_FE_URL || 'http://localhost:5173') + '/product/99999');
